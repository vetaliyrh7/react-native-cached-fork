...
class OfflineImageStore {
  ...
  async restore(config, onRestoreCompletion) {
    ...
    AsyncStorage.getItem(`@${this.store.name}:uris`, (err, uris) => {
      // On `getItems` completion
  ...
  _removeExpiredImages = onRestoreCompletion => {
    ...
    uriListToRemove.forEach(uri => {
      ...
      const unlinkPromise = RNFetchBlob.fs
        .unlink(this.entries[uri].basePath + '/' + this.entries[uri].localUriPath)
        .then(() => {
    ...
    if (toBeRemovedImagePromises.length > 0) {
      ...
      // Nothing to remove so just trigger callback!
      if (this.store.debugMode) {
  ...
  _downloadImage = source => {
    ...
    RNFetchBlob.config({
      ...
      .then(() => {
        ...
        this._updateOfflineStore(source.uri)
      })
      .catch((err) => {
        console.log('err', err)
        source.callback({
